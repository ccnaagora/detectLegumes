...
def dessinerDirection2(image , seuils , contour):
...
def dessinerDirection(image , seuils ):
    hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
    ...
    # 2. Sélectionner le contour de l'objet (ex: le plus grand)
    if contours:
        largest_contour = max(contours, key=cv2.contourArea)
        # 3. Calculer le centroïde
        M = cv2.moments(largest_contour)
        ...
        if M["m00"] != 0:
            ...
            cY = int(M["m01"] / M["m00"])
            # 4. Préparer les points du contour pour PCACompute
            points = largest_contour[:, 0, :]  # Récupérer les points (format initial : int)
            points = points.astype(np.float32)  # Convertir en float32
            # 5. Calculer la PCA
            mean, eigenvectors = cv2.PCACompute(points, mean=None)
            # 6. Calculer l'angle d'orientation
